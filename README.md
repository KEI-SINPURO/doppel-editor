...
- 効果音の「AIにおまかせ」は、組み込みプリセット（6種類）の中からの使い分けのみに対応しています。
  自分でアップロードした複数のSE素材から場面ごとに選ばせることは、現バージョンでは未対応です
- BGMの自動ダッキングは、セリフ区間（発話セグメント単位）の一律の音量制御であり、
  声の大きさ・BGMの盛り上がり等を検知した高度なオートミックスではありません
...
## 12. 動画編集そのものの機能強化（プロの編集で当たり前だが未実装だった部分）

AIの「判断力」だけでなく、実際の編集処理自体で足りていなかった部分も補いました。
いずれも追加の環境変数設定は不要です。

### 効果音（SE）を場面ごとにAIが使い分ける

「動画を再現する」タブ・バッチ処理タブの演出オプションで、効果音の選択肢に
**「🤖 AIにおまかせ（場面ごとに種類を使い分け）」** を追加しました。選択すると、
AIがハイライト瞬間ごとに「嬉しい発見」「衝撃」「場面転換」「軽いツッコミ」「失敗・警告」
「達成・締めくくり」の6種類から最も合う効果音を判断し、`features/se_presets.py` の
組み込みプリセットの中から自動で使い分けます（`ai/model.py` の `se_mood`、
`features/branding.py` の `insert_multiple_se`）。
従来は動画全体で1種類の効果音しか使えませんでした。
AIが使えない場合（ルールベース判定時）は、代わりに全て「ドン（impact）」を使います。

### BGMの自動ダッキング

`features/branding.py` の `mix_bgm()` に、セリフ区間だけBGM音量を自動で下げる
「ダッキング」機能を追加しました（プロの編集で当たり前に行われる基本テクニック）。
「動画を再現する」でBGMを設定すると、**追加設定なしで常に適用されます**。
以前はBGMが動画全体で一定音量のままミックスされ、セリフに被って聞き取りにくくなる
ことがありました。

### 概要欄・チャプターの自動生成

「動画を再現する」タブ、最終レンダリング完了後に「📋 概要欄・チャプターも作る」という
折りたたみセクションが追加されました。ボタンを押すと、実際に完成動画で使われた発言から、
YouTube投稿用の概要欄の文章と、タイムスタンプ付きチャプター（`0:00` から始まる形式）を
AIが自動生成します（`ai/model.py` の `generate_video_description`）。生成結果はテキスト
エリアに表示されるので、そのままコピーしてYouTubeの概要欄に貼り付けられます。
学習済みのテロップの文体（`subtitle_voice`）があれば、概要欄の文体もそれに寄せます。